...
const isMobileSafari = Utils.isMobileSafari();
if (!isMobileSafari) SceneManager._currentTime = SceneManager._getTimeInMsWithoutMobileSafari();
...
BattleManager.invokeAction = function(subject, target) {
    ...
    if (Math.random() < this._action.itemCnt(target)) {
        ...
        if(target.mdf >= subject.mat){
            this.invokeMagicReflection(subject, target);
          }else{
            TickerManager.show(`威力过强，反射失败!`);
