...
Game_Action.prototype.itemHit = function(target) {
    ...
    if (this.subject().isActor() && target.isStateAffected(141) && !this.isForAll()){a *= 0.33;}
    if (!this.subject().isActor() && target.isStateAffected(86) && !this.isForAll()){a *= 0.5;}
...
Game_Action.prototype.itemEva = function(target) {
    if(target.isStateAffected(142) || target.isStateAffected(143) || target.isStateAffected(89)){
        if(this.isCertainHit() && !this.isDamage()){
            return 0;
        }
        return 1;
    }
    if (this.isPhysical()) {
        if(target.isStateAffected(236)){return 1;}
        if(this.subject().agi <= 1.2 * (target.agi)){
            return (target.eva + 0.95 - this.subject().hit);
        }else{
            TickerManager.show(`难以回避的速度!`);
            return ((target.eva / 2) + 0.95 - this.subject().hit);
        }

    } else if (this.isMagical()) {
        if(this.subject().mat <= 1.5 * (target.mdf)){
            return target.mev;
        }else{
            TickerManager.show(`无法抵挡的超强力魔法!`);
            return 0;
        }
...
Game_BattlerBase.prototype.xparam = function(xparamId) {
	if(xparamId == 9 && this.isEnemy()) return this.traitsSum(Game_BattlerBase.TRAIT_XPARAM, xparamId) + 0.04 + 0.01 * $gameVariables.value(4980); 
...
Game_BattlerBase.prototype.canPaySkillCost = function(skill) {
    if (skill.id == 75 && this._tp >= 30) return true;
    if (skill.id == 77 && this._tp >= 30) return true;
    if (skill.id == 85 && this._tp >= 30) return true;
...
Game_BattlerBase.prototype.paySkillCost = function(skill) {
    ...
    if (skill.id == 66 || skill.id == 61 || skill.id == 56) {
        this._mp -= this.param(1) * 0.08 * this.mcr;;
    }
    if (skill.id == 67 || skill.id == 62 || skill.id == 57 || skill.id == 76) {
        this._mp -= this.param(1) * 0.18 * this.mcr;;
    }
    if (skill.id == 51) {
        this._mp -= this.param(1) * 0.12 * this.mcr;;
    }
    if (skill.id == 52) {
        this._mp -= this.param(1) * 0.27 * this.mcr;;
    }
    if (skill.id == 75 || skill.id == 85 || skill.id == 77){
        this._tp -= 100;
    }
    if (skill.id == 39) {
        this._tp -= 60;
...
Game_Battler.prototype.regenerateHp = function() {
    ...
    if (value !== 0 && !this.isStateAffected(14)) {
        this.gainHp(value);
    }
};
...
Game_Enemy.prototype.paramBase = function(paramId) {
    ...
    case 2:
        ...
        break;
	case 3:
        return this.enemy().params[paramId] * [2, 1.2, 1.5, 1.5, 1.5, 1.5, 1.5, 1][paramId];
        break;
	case 4:
        return this.enemy().params[paramId] * [2.5, 1.2, 2, 2, 2, 2, 2, 1][paramId];
        break;
    default:
...
Game_Party.prototype.allMembers = function() {
    return Array.isArray(this._actors)? this._actors.map(function(id) {
