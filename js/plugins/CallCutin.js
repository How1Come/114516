...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (command === 'CallCutin') {
      ...
      var PotionFlag = 0; //Cutin1Potion1
  ...
  s.opacity = (($gameVariables.value(1103)+$gameVariables.value(1074)*0.25+$gameVariables.value(1072)*0.5)/20*255).clamp(0,255);
      ...
      if (HairFlag >= 1) {//头发换色       
        if (CLOTHTAG === "EvilChange") //魔人
        {
          FILENAME = "actor01_cutin_" + CUTINBASENUM + "_000" + CUTINFILENUM;
        }
        else if(balckFlag)//黑皮
        {
          CUTINFILENUM = CUTINFILENUM.slice(0, -1); 
      ...
      //身上带的媚药
      if (PotionFlag >= 1 && $gameVariables.value(4845) > 0) {
