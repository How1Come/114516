...
(function () {
	...
	Game_Interpreter.prototype.pluginCommand = function (command, args) {
		...
		if (command === 'CallStand' || command === 'CallStandForce') {
			...
			let CosplayFlag = 0
			//发型样式的确定
			let hairStyle = (StandAltFlag || Cosplay == 1)? 2 : 1 //默认为未变身的样式 1未变身2变身
			if (user.hairStyle && hairStyle != 1) hairStyle = user.hairStyle[hairStyle - 1] //用户自定义发型样式
			//发饰确定
			const hairDress = [1, 2, 3][hairStyle - 1]//未变身，变身，高马尾
			//处理部分衣服的发饰颜色差异
			user.hairDtone = hairStyle !== 1 && $dataArmors[StandEqNum].meta.ChangeHairD ? $dataArmors[StandEqNum].meta.ChangeHairD : null;
			...
			var [
				...
				effect_breath,
				stand_diffront] = Array(39).fill(null).map((_, i) => i); //diffront在下面reset_ERO
			//动图层数量必须改数组长度
			var stand_array = Array(39).fill(null).map((_, i) => i + 110);
			...
			if ((hairDress != 1 && StandPoseID == 6) || (hairDress == 1 && (StandPoseID == 6))) {
				if ($gameScreen.picture(113)) $gameScreen.erasePicture(113);
			...
			if (ClothPicFileNum != 0 && heartNipple == 0) {
				...
				checkFileName = "cloth/05_00" + (ClothPicFileNum.replace(/\D/g, '').length > 1 ? "" : "0") + ClothPicFileNum
				if ($gameScreen.picture(stand_cloth + 1) && $gameScreen.picture(stand_cloth + 1)._name == checkFileName && ClothUpdate == 0) $gameScreen.erasePicture(stand_cloth + 1)
				if ($gameScreen.picture(stand_cloth + 2) && $gameScreen.picture(stand_cloth + 2)._name == checkFileName && ClothUpdate == 0) $gameScreen.erasePicture(stand_cloth + 2)
			...
			//永恒发光表情
			if ([5, 13, 15, 35].includes(FaceId) && (user.isStateAffected(88) || user.isStateAffected(428))) FaceId += "a";
			...
			if (EarPicFileNum == 0) { $gameScreen.erasePicture(stand_ear); }
				...
				{
					switch (FaceId) {
						case 51:FaceId += "a";break;
						case 52:FaceId += "b";break;
						case 53:FaceId += "c";break;
						case 54:FaceId += "d";break;
						case 55:FaceId += "e";break;
						case 56:FaceId += "f";break;
					}
				}
				console.log(stand_tail)
