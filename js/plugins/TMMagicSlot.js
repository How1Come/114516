 ...
 * @default 最大%1
 * 
 * @param explainFontSize
 * @type number
 * @desc 取得可能なスキルの説明のフォントサイズ
 * @default 14
 * 
 * @param explainMagicColor
 * @type number
 * @desc 取得可能なスキルが魔法だった場合の色
 * @default 23
 * 
 * @param explainPassiveColor
 * @type number
 * @desc 取得可能なスキルがパッシブだった場合の色
 * @default 20
 * 
 * @param explainSkillMargin
 * @type number
 * @desc 取得可能なスキル間のマージン
 * @default 18
 *
 ...
 *
 *   setAdditionalMagicSlot 1 3
 *     アクター 1 番の追加空きスロットを 3 にします。
 *     数値には制御文字 \V[n] が使用できます。
 * 
 *   clearMagicEquip 1
 *     アクター 1番の装備魔法をすべてはがします
 * 
 *
 * プラグインパラメータ補足:
 *
...
TMPlugin.MagicSlot.ExplainFontSize = parseInt(TMPlugin.MagicSlot.Parameters['explainFontSize']);
TMPlugin.MagicSlot.ExplainPassiveColor = parseInt(TMPlugin.MagicSlot.Parameters['explainPassiveColor']);
TMPlugin.MagicSlot.ExplainMagicColor = parseInt(TMPlugin.MagicSlot.Parameters['explainMagicColor']);
TMPlugin.MagicSlot.ExplainSkillMargin = parseInt(TMPlugin.MagicSlot.Parameters['explainSkillMargin']);
...
(function() {
	...
	Game_Actor.prototype.addMagicSlot = function(value) {
		...
		if(this._magicSlotPlus < 0) this._magicSlotPlus = 0;
		this.refresh();
	};

	Game_Actor.prototype.setAdditionalMagicSlot = function(value) {
		this._magicSlotPlus = value;
		this.refresh();
	};

	Game_Actor.prototype.isEquipedMagic = function(itemId) {
		if(!this._magics) return;
		return this._magics.includes(itemId);
	};
	...
	Game_Interpreter.prototype.pluginCommand = function(command, args) {
		...
		} else if (command === 'setAdditionalMagicSlot') {
			var arr = args.map(this.convertEscapeCharactersTM, this);
			var actor = $gameActors.actor(+arr[0]);
			if (actor) {
				actor.setAdditionalMagicSlot(+arr[1]);
			}
		} else if (command === 'clearMagicEquip') {
			var arr = args.map(this.convertEscapeCharactersTM, this);
			var actor = $gameActors.actor(+arr[0]);
			if (actor) {
				actor.clearMagics();
	...
	function Window_MagicExplain() {
		this.initialize.apply(this, arguments);
	}

	Window_MagicExplain.prototype = Object.create(Window_Base.prototype);
	Window_MagicExplain.prototype.constructor = Window_MagicExplain;

	Window_MagicExplain.prototype.initialize = function(x, y, width, height) {
		Window_Base.prototype.initialize.call(this, x, y, width, height);
	};

	Window_MagicExplain.prototype.setItem = function(item) {
		if(this.item != item){
			this.item = item;
			this.refresh();
		}
	...
	Window_MagicExplain.prototype.refresh = function() {
		this.contents.clear();
		if (!this.item || !this.item.traits) return;
		var y = 0;
		for(var t of this.item.traits){
			if(t.code != Game_BattlerBase.TRAIT_SKILL_ADD) continue;
			this.resetFontSettings();
			var skill = $dataSkills[t.dataId];
			if(!skill) continue;

			if(skill.stypeId == 1)this.changeTextColor(this.textColor(TMPlugin.MagicSlot.ExplainMagicColor));
			else if(skill.stypeId == 4)this.changeTextColor(this.textColor(TMPlugin.MagicSlot.ExplainPassiveColor));
			this.drawText(skill.name, 0, y);
			y += this.lineHeight() - 5;
			y = this.drawDescription(skill.description, y);
			y += TMPlugin.MagicSlot.ExplainSkillMargin;
		}
	};

	Window_MagicExplain.prototype.drawDescription = function(desc, y) {
		this.contents.fontSize = TMPlugin.MagicSlot.ExplainFontSize;
		var text = '';
		var indent = 10;
		var width = this.contentsWidth() - indent - this.contents.fontSize;
		for(var i = 0; i < desc.length; i++){
			text += desc[i];
			if(text.length > 0 && (i == desc.length - 1 || this.textWidth(text) >= width)){
				this.drawText(text, indent, y);
				text = '';
	...
	Window_MagicStatus.prototype.numVisibleRows = function() {
		return TMPlugin.MagicSlot.MaxMpText ? 2 : 1;
	};
	...
	Window_MagicSlot.prototype.setExplainWindow = function(explainWindow) {
		this._explainWindow = explainWindow;
	};

	Window_MagicSlot.prototype.updateHelp = function() {
		Window_Selectable.prototype.updateHelp.call(this);
		this.setHelpWindowItem(this.item());
		this._explainWindow.setItem(this.item());
	};
	...
	Window_MagicItem.prototype.setExplainWindow = function(explainWindow) {
		this._explainWindow = explainWindow;
	};

	var _Window_MagicItem_updateHelp = Window_MagicItem.prototype.updateHelp;
	Window_MagicItem.prototype.updateHelp = function() {
		_Window_MagicItem_updateHelp.call(this);
		this._explainWindow.setItem(this.item());
	};

	Window_MagicItem.prototype.numberWidth = function() {
		return this.textWidth('0');
	};
	
	Window_MagicItem.prototype.needsNumber = function() {
		return false;
	};
	...
	Scene_MagicEquip.prototype.createMagicExplainWindow = function() {
		var y = this._helpWindow.height + this._statusWindow.height;
		this._explainWindow = new Window_MagicExplain(0, y, this._statusWindow.width, Graphics.height - y);
		this.addWindow(this._explainWindow);
		this._itemWindow.setExplainWindow(this._explainWindow);
		this._slotWindow.setExplainWindow(this._explainWindow);
	};
	...
	Scene_MagicEquip.prototype.onSlotCancel = function() {
		...
		this._commandWindow.activate();
		this._explainWindow.setItem(null);
	};

	Scene_MagicEquip.prototype.onItemOk = function() {
		if($gameActors.actor(1)._classId == 4){
			TickerManager.show(`魔人状态无法装备印记`);
		    this._slotWindow.activate();
		    this._slotWindow.refresh();
