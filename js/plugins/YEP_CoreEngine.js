...
Sprite.prototype.updateTransform = function() {
  ...
  //为整数则不进行取整
  if (this.worldTransform.tx % 1 !== 0 || this.worldTransform.ty % 1 !== 0) {
    this.worldTransform.tx = Math.floor(this.worldTransform.tx);
    this.worldTransform.ty = Math.floor(this.worldTransform.ty);
}
};
...
Window.prototype._roundWhUp = function() {
  ...
  this._width |= 0; // 通过按位或操作将浮点数部分截断
  this._height |= 0; // 同样的操作应用于高度
};
...
Game_Actor.prototype.paramMax = function(paramId) {
  if (paramId === 0) {
    if(this._classId == 1){return 500;}
      return Yanfly.Param.ActorMaxHp;
  } else if (paramId === 1) {
    if(this._classId == 1){return 200;}
      return Yanfly.Param.ActorMaxMp;
  } else {
    if(this._classId == 1){return 100;}
...
Game_Map.prototype.updateEvents = function() {
    ...
    // }
    // 获取地图事件和公共事件的合并数组
    var events = this.events().concat(this._commonEvents);
    // 获取数组长度，避免每次迭代都重新计算
