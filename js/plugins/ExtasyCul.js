    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
      ...
      if (command === 'ExtasyCul') {
			...
			if($gameParty.inBattle()){
                if(ExtasyPointTotal > 150){
                    TickerManager.show(`猛烈的快感让天音站不稳了`);
                    $gameActors.actor(1).addState(433);
                }
                if(ExtasyPointTotal > 50){
                    TickerManager.show(`强烈的快感让天音使不上劲`);
                    $gameActors.actor(1).addState(440);
                }
