...
(function() {
        ...
        if (command === 'EnemyBook') {
            ...
            case 'add':
                    if (args[1].match(/\\v/)) {
                        ...
                        args[1] = Number(array);
                        var AddId = $gameVariables.value(args[1]);
                          }
                        }else{var AddId = Number(args[1])
                          }
                $gameSystem.addToEnemyBook(AddId);
                break;
            case 'remove':
    ...
    Window_EnemyBookStatus.prototype.refresh = function() {
        ...
        for (var i = 0; i < 7; i++) {//luc表示しないように改変
            this.changeTextColor(this.systemColor());
            this.drawText(TextManager.param(i), x, y, 160);
            this.resetTextColor();
            if (i == 0) {
                this.drawText(enemy.params[i] * m <= 999999? Math.floor(enemy.params[i] * m): 999999, x + 160, y, 60, 'right');
            }else if(i == 1){
                this.drawText(enemy.params[i] * n <= 9999? Math.floor(enemy.params[i] * n): 9999, x + 160, y, 60, 'right');
            }else{
                this.drawText(enemy.params[i] * b <= 999? Math.floor(enemy.params[i] * b): 999, x + 160, y, 60, 'right');
            }
            y += lineHeight;
        }
